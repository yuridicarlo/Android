...
import it.bancomat.pay.consumer.extended.flowmanager.SettingsFlowManagerExtended;
...
public class SettingsForceLoginActivity extends AppGenericErrorActivity implements LabelPasswordLogin.LabelListener {
        ...
        Task<?> t = BancomatPayApiInterface.Factory.getInstance().doVerifyPin(result -> {
            if (result != null) {
                if (result.isSuccess()) {
                    ...
                    boolean isRequired = getIntent().getBooleanExtra(SettingsFlowManagerExtended.ENABLE_FORCE_LOGIN, false);
                    BancomatPayApiInterface.Factory.getInstance().setLoginRequired(isRequired);
                    finish();
